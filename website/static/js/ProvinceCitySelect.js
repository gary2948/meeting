...
(function($) {
    $.fn.ProvinceCitySelect = function(option, value) {
        if (typeof option === "string") {
            if (value == undefined) {
                return $(this[0]).data("ProvinceCitySelect")[option]()
            }
            return $(this[0]).data("ProvinceCitySelect")[option](value)
        } else if (typeof option === "object") {
            return $(this).each(function() {
                ProvinceCitySelect($(this), option)
            })
        }
    }

    function ProvinceCitySelect(Dom, option) {
        var defaultsetting = DefaultSetting();
        var Setting = $.extend({}, defaultsetting, option);
        var divProvince = CreateItems(Setting.Address, Setting.Province, ProvinceItemClick);
        var divCity;
        var citys = ReturnProvince(Setting.Province);
        if (citys != null) {
            divCity = CreateItems(citys.cityList, Setting.City, CityItemClick);
        } else {
            divCity = CreateItems([], Setting.City, CityItemClick);
        }
        $(Dom).append($('<div class="btn-group"><button class="btn dropdown-toggle disable" data-toggle="dropdown">中国</button></div>'))

        $(Dom).append(divProvince).append(divCity);

        BindFunction()
        $(Dom).data('ProvinceCitySelect', Setting)
        ...
        function CreateItems(ItemList, value, ItemClickFunction) {
            ...
            var divButtonValue = $("<span></span>").html(value).addClass("pickValue");
            var divButtonCaret = $("<span></span>").addClass("caret").html("");
            var divUl = $("<ul></ul>").addClass("dropdown-menu").css({
                "height": "300px",
                "overflow-y": "auto"
            });
            ...
            for (var i = 0; i < ItemList.length; i++) {
                li = $("<li></li>").append($("<a href='javascript:void(0)'></a>").html(ItemList[i].name)).data("ItemValue", ItemList[i]).click(function(event) {
                    var iv = $(this).data("ItemValue");
                    divButtonValue.html(iv.name).data("value", iv);
                    ItemClickFunction(this, event);
                });
        ...
        function ProvinceItemClick(sender, event) {
            var value = $(sender).data("ItemValue");
            Setting.Province = value.name;
            divCity.remove();
            divCity = CreateItems(value.cityList, defaultsetting.City, CityItemClick);
            $(Dom).append(divCity);
        }

        function CityItemClick(sender, event) {
            var value = $(sender).data("ItemValue");
            Setting.City=value.name;
        }

        function AreaItemClick() {}

        function ReturnProvince(value) {
            for (var i = 0; i < Setting.Address.length; i++) {
                if (Setting.Address[i].name == value)
                    return Setting.Address[i];
            }
            return null;
        }

        function GetProvince() {
            return Setting.Province;
        }
        ...
        function GetCombo(value){
            return Setting.Province+value+Setting.City;
        }

        function BindFunction(){
            Setting.GetProvince=GetProvince;
            Setting.GetCity=GetCity;
            Setting.GetCombo=GetCombo;
        }
